...
void turnMat(const int n, const int maxNNZ, const floatType* data, const int* indices, const int* length, floatType* data_turn, int* indices_turn){
	int i,j;
	#pragma omp parallel for default(none) shared(data,data_turn,indices,indices_turn,length) private(i,j)
	for(i = 0; i < n; i++){
		for(j = 0; j < length[i]; j++){
			data_turn[(i*maxNNZ) + j] = data[i + (j*n)];
			indices_turn[(i*maxNNZ) + j] = indices[i + (j*n)];
		}
	}
}

void getReciprocalMainDiagonal(const int n, const int maxNNZ, const floatType* data, const int* indices, const int* length, floatType* data_diag){
	int i,j;
...
int main(int argc, char *argv[]){
	...
	/* Now turn the matrix and around so we got rows first */  
	floatType* data_turn = NULL;
        int* indices_turn = NULL;

        data_turn = (floatType*)malloc((maxNNZ*n)*sizeof(floatType));
        indices_turn = (int*)malloc((maxNNZ*n)*sizeof(int));
	
	printf("Turning the Matrix\n");
	turnMat(n,maxNNZ,data,indices,length,data_turn,indices_turn);
	printf("Turning done\n");

	/* Get the reciprocal main diagonal for Jacobi Preconditioner */
	printf("Getting reciprocal main diagonal\n");
	floatType* data_diag = (floatType*) malloc(n * sizeof(floatType));
	getReciprocalMainDiagonal(n, maxNNZ, data, indices, length, data_diag);
	printf("Reciprocal main diagonal done\n");
	 ...
	 * competition. */
	solveTime = getWTime();
	cg(n, nnz, maxNNZ, data_turn, indices_turn, length, data_diag, b, x, &sc);
	...
	floatType gflops = (2.0 * sc.iter * nnz) / sc.timeMatvec / 1000000000;
